...
def optimize_graph_edit_distance(G1, G2, node_match=None, edge_match=None,
    ...
    for vertex_path, edge_path, cost in \
        optimize_edit_paths(G1, G2, node_match, edge_match,
                            node_subst_cost, node_del_cost, node_ins_cost,
                            edge_subst_cost, edge_del_cost, edge_ins_cost,
                            upper_bound, strictly_decreasing):
        yield cost
...
def optimize_edit_paths(G1, G2, node_match=None, edge_match=None,
    ...
    def google_wrapper(C):
        lsa_row_ind = []
        lsa_col_ind = []
        assignment = pywrapgraph.LinearSumAssignment()
        for source in range(0, C.shape[0]):
            for target in range(0, C.shape[1]):
                assignment.AddArcWithCost(source, target, int(C[source][target]))

        solve_status = assignment.Solve()
        if solve_status == assignment.OPTIMAL:
            for i in range(0, assignment.NumNodes()):
                j = assignment.RightMate(i)
                lsa_row_ind.append(i)
                lsa_col_ind.append(j)
        elif solve_status == assignment.INFEASIBLE:
            print('No perfect matching exists.')
        elif solve_status == assignment.POSSIBLE_OVERFLOW:
            print('Some input costs are too large and may cause an integer overflow.')
        
        lsa_row_ind = np.array(lsa_row_ind, dtype=np.int)
        lsa_col_ind = np.array(lsa_col_ind, dtype=np.int)
        return lsa_row_ind, lsa_col_ind
    
    def munkres_wrapper(C):
        lsa_row_ind = []
        lsa_col_ind = []
        R = munkres(C.copy(order='C'))
    ...
    def greedy_wrapper(cost):
       ...
       unique_rows = np.array(unique_rows, dtype=np.int)
       unique_cols = np.array(unique_cols, dtype=np.int)

       return unique_rows, unique_cols
    
    # def greedy_wrapper(z):
    ...
    #         lsa_row_ind.append(index[0])
    #         lsa_col_ind.append(index[1])
    #     lsa_row_ind = np.array(lsa_row_ind, dtype=np.int)
    #     lsa_col_ind = np.array(lsa_col_ind, dtype=np.int)
    #     return lsa_row_ind, lsa_col_ind
    ...
    def make_CostMatrix(C, m, n):
        ...
        # lsa_row_ind, lsa_col_ind = google_wrapper(C)
        lsa_row_ind, lsa_col_ind = munkres_wrapper(C)
        # lsa_row_ind, lsa_col_ind = greedy_wrapper(C)
        
        # print(lsa_row_ind, lsa_col_ind)
        # print(lsa_row_ind_c, lsa_col_ind_c)
